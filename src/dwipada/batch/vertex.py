...
from dwipada.paths import PROJECT_ROOT, OUTPUT_DIR
from dwipada.batch.config import load_vertex_config
...
def cmd_upload(args):
    ...
    model = vertex_cfg.get("model", DEFAULT_MODEL)
    ...
    print(f"\nTo check status:  python -m dwipada.batch.vertex --status \"{job.name}\"")
    print(f"To get results:   python -m dwipada.batch.vertex --results \"{job.name}\"")
...
def cmd_results(args):
    ...
    if args.output:
        ...
        local_out = DEFAULT_OUTPUT_DIR / "batch_responses_vertex.jsonl"
...
Examples:
  ...
  python -m dwipada.batch.vertex --results "projects/123/.../456" -o output/batch_responses_100.jsonl
