#!/usr/bin/env python3
"""
Benchmark: Hybrid (Logit Masking + NFA Rejection Sampling).

Approach:
    1. Same logit masking as masking_only (static + dynamic Gana NFA mask)
       narrows ~256K vocab to ~1000 valid Telugu tokens.
    2. From the masked logits, take the top-100 candidates by probability.
    3. For each candidate, clone the CompositeState, feed the token, flush,
       and check is_alive() + has_accept() via the incremental NFA.
    ...
    6. Falls back to backtracking only if NO valid candidate in top-100.

Expected validity: ~100% on gemma3-1b-merged.

This combines the best of both worlds:
  - Masking eliminates non-viable tokens efficiently (no wasted forward passes)
  - NFA rejection ensures every committed token actually progresses toward
    a valid gana partition (not just "reachable")
  - The accept-candidate priority steers the model toward line completion

Usage:
    python domino/benchmark_hybrid.py --model gemma3-1b-merged --seeds 20 --topic "శ్రీరాముడు"
"""
...
SEARCH_K = 100              # how many top candidates to test per step

# Backtracking constants (fallback — rarely needed)
...
###############################################################################
# GENERATION — HYBRID (MASK + NFA REJECTION)
###############################################################################
...
def generate_poem(
    ...
    """
    Generate a dwipada poem using hybrid masking + NFA rejection sampling.

    At each step:
      1. Forward pass → logits.
      2. Apply static mask (kill non-Telugu) + dynamic Gana NFA mask.
      3. Take top-SEARCH_K candidates from masked logits.
      4. For each candidate: clone CompositeState, feed token, flush, check
         is_alive() and has_accept() via the incremental NFA.
      5. Prefer accept candidates; weighted sample from best set.
      6. If no valid candidate found, backtrack to checkpoint.
    ...
    with torch.no_grad():
        ...
        while step < max_new_tokens:
            ...
            if not prefix_done:
                # Before prefix: simple top-p sampling
                ...
                # NFA rejection: test top-K candidates from masked logits
                chosen_id = None
                valid_candidates = []
                accept_candidates = []
                snap = composite.snapshot()
                ...
                for k in range(min(SEARCH_K, len(sorted_indices))):
                    ...
                    # Stop early with accept candidates
                ...
                if use_candidates and chosen_id is None:
                    ...
                    # No valid candidate — backtrack (rare)
